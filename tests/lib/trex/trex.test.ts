...
describe("TRex", () => {
  describe("find", () => {
    ...
    test("should handle surrogate pairs correctly", () => {
      const source = StrSlice.from("abc 😀 def");
      const matcher = new MatchCodePoint('😀'.codePointAt(0)!);
      const trex = new TRex(matcher);
  ...
  describe("findAll", () => {
    test("should find all matches in the source string", () => {
      ...
      expect(tokens[2].value.value).toBe(" def ");
      expect(tokens[3].kind).toBe(":match");
      expect(tokens[3].value.value).toBe("xxx");
      expect(tokens[4].kind).toBe(":fragment");
      expect(tokens[4].value.value).toBe(" ghi");
    });
    ...
    test("should handle case with no matches", () => {
      ...
      expect(tokens[0].kind).toBe(":fragment");
      expect(tokens[0].value.value).toBe("abc def ghi");
    });
    ...
    test("should find words with MatchWord and MatchAnyString", () => {
      const str = "abc def xxx hij yyy lmn opq xxx yyz xxx yyy mmm cba xxxyyy yyyxxx yyy";
      ...
      expect(tokens.filter(t => t.kind === ":match")).toHaveLength(6);
      
      // Check specific matches
      const matches = tokens.filter(t => t.kind === ":match").map(t => t.value.value);
      expect(matches).toEqual(["xxx", "yyy", "xxx", "xxx", "yyy", "yyy"]);
      
      // Check that the fragments are correct
      const fragments = tokens.filter(t => t.kind === ":fragment").map(t => t.value.value);
      expect(fragments[0]).toBe("abc def ");
      expect(fragments[1]).toBe("hij ");
      expect(fragments[2]).toBe("lmn opq ");
      expect(fragments[3]).toBe("yyz ");
      expect(fragments[4]).toBe(" mmm cba xxxyyy yyyxxx ");
    });
  });
});
